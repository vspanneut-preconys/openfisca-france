...
class cappme(Variable):
    ...
    def formula_2015_01_01(self, simulation, period):
        ...
        f7cr = simulation.calculate('f7cr', period)
        f7cu = simulation.calculate('f7cu', period)
        f7cv = simulation.calculate('f7cv', period)
        f7cy = simulation.calculate('f7cy', period)
    ...
    def formula_2016_01_01(self, simulation, period):
        ...
        f7cu = simulation.calculate('f7cu', period)
        f7cv = simulation.calculate('f7cv', period)
        f7cx = simulation.calculate('f7cx', period)
        f7cy = simulation.calculate('f7cy', period)
        f7dy = simulation.calculate('f7dy', period)
...
class doment(Variable):
    ...
    def formula_2013_01_01(self, simulation, period):
        ...
        fhks = simulation.calculate('fhks', period)
        fhkt = simulation.calculate('fhkt', period)
        fhku = simulation.calculate('fhku', period)
        fhlg = simulation.calculate('fhlg', period)
        fhlh = simulation.calculate('fhlh', period)
        fhli = simulation.calculate('fhli', period)
        fhma = simulation.calculate('fhma', period)
        fhmb = simulation.calculate('fhmb', period)
        fhmc = simulation.calculate('fhmc', period)
        fhmm = simulation.calculate('fhmm', period)
        fhmn = simulation.calculate('fhmn', period)
        fhnu = simulation.calculate('fhnu', period)
        fhnv = simulation.calculate('fhnv', period)
        fhnw = simulation.calculate('fhnw', period)
        fhny = simulation.calculate('fhny', period)
        fhpa = simulation.calculate('fhpa', period)
        fhpb = simulation.calculate('fhpb', period)
        fhpd = simulation.calculate('fhpd', period)
        fhpe = simulation.calculate('fhpe', period)
        fhpf = simulation.calculate('fhpf', period)
        fhph = simulation.calculate('fhph', period)
        fhpi = simulation.calculate('fhpi', period)
        fhpj = simulation.calculate('fhpj', period)
        fhpl = simulation.calculate('fhpl', period)
        fhpm = simulation.calculate('fhpm', period)
        fhpn = simulation.calculate('fhpn', period)
        fhpo = simulation.calculate('fhpo', period)
        fhpp = simulation.calculate('fhpp', period)
        fhpr = simulation.calculate('fhpr', period)
        fhps = simulation.calculate('fhps', period)
        fhpt = simulation.calculate('fhpt', period)
        fhpu = simulation.calculate('fhpu', period)
        fhpw = simulation.calculate('fhpw', period)
        fhpx = simulation.calculate('fhpx', period)
        fhpy = simulation.calculate('fhpy', period)
        fhqe = simulation.calculate('fhqe', period)
        fhqf = simulation.calculate('fhqf', period)
        fhqg = simulation.calculate('fhqg', period)
        fhqi = simulation.calculate('fhqi', period)
        fhqo = simulation.calculate('fhqo', period)
        fhqp = simulation.calculate('fhqp', period)
        fhqr = simulation.calculate('fhqr', period)
        fhqv = simulation.calculate('fhqv', period)
        fhqz = simulation.calculate('fhqz', period)
        fhrg = simulation.calculate('fhrg', period)
        fhri = simulation.calculate('fhri', period)
        fhrj = simulation.calculate('fhrj', period)
        fhrk = simulation.calculate('fhrk', period)
        fhrl = simulation.calculate('fhrl', period)
        fhrm = simulation.calculate('fhrm', period)
        fhro = simulation.calculate('fhro', period)
        fhrp = simulation.calculate('fhrp', period)
        fhrq = simulation.calculate('fhrq', period)
        fhrr = simulation.calculate('fhrr', period)
        fhrt = simulation.calculate('fhrt', period)
        fhru = simulation.calculate('fhru', period)
        fhrv = simulation.calculate('fhrv', period)
        fhrw = simulation.calculate('fhrw', period)
        fhry = simulation.calculate('fhry', period)
    ...
    def formula_2014_01_01(self, simulation, period):
        ...
        Investissements dans les DOM-TOM dans le cadre d'une entrepise.
        '''
        fhaa = simulation.calculate('fhaa', period)
        fhab = simulation.calculate('fhab', period)
        fhac = simulation.calculate('fhac', period)
        fhae = simulation.calculate('fhae', period)
        fhaf = simulation.calculate('fhaf', period)
        fhag = simulation.calculate('fhag', period)
        fhah = simulation.calculate('fhah', period)
        fhaj = simulation.calculate('fhaj', period)
        fhak = simulation.calculate('fhak', period)
        fhal = simulation.calculate('fhal', period)
        fham = simulation.calculate('fhal', period)
        fhao = simulation.calculate('fhao', period)
        fhap = simulation.calculate('fhap', period)
        fhaq = simulation.calculate('fhaq', period)
        fhar = simulation.calculate('fhar', period)
        fhat = simulation.calculate('fhat', period)
        fhau = simulation.calculate('fhau', period)
        fhav = simulation.calculate('fhav', period)
        fhaw = simulation.calculate('fhaw', period)
        fhay = simulation.calculate('fhay', period)
        fhba = simulation.calculate('fhba', period)
        fhbb = simulation.calculate('fhbb', period)
        fhbe = simulation.calculate('fhbe', period)
        fhbg = simulation.calculate('fhbg', period)
        fhks = simulation.calculate('fhks', period)
        fhkt = simulation.calculate('fhkt', period)
        fhku = simulation.calculate('fhku', period)
        fhlg = simulation.calculate('fhlg', period)
    ...
    def formula_2015_01_01(self, simulation, period):
        ...
        fhbe = simulation.calculate('fhbe', period)
        fhbg = simulation.calculate('fhbg', period)
        fhbi = simulation.calculate('fhbi', period)
        fhbj = simulation.calculate('fhbj', period)
        fhbk = simulation.calculate('fhbk', period)
        fhbm = simulation.calculate('fhbm', period)
        fhbn = simulation.calculate('fhbn', period)
        fhbo = simulation.calculate('fhbo', period)
        fhbp = simulation.calculate('fhbp', period)
        fhbr = simulation.calculate('fhbr', period)
        fhbs = simulation.calculate('fhbs', period)
        fhbt = simulation.calculate('fhbt', period)
        fhbu = simulation.calculate('fhbu', period)
        fhbw = simulation.calculate('fhbw', period)
        fhbx = simulation.calculate('fhbx', period)
        fhby = simulation.calculate('fhby', period)
        fhbz = simulation.calculate('fhbz', period)
        fhcb = simulation.calculate('fhcb', period)
        fhcc = simulation.calculate('fhcc', period)
        fhcd = simulation.calculate('fhcd', period)
        fhce = simulation.calculate('fhce', period)
        fhcg = simulation.calculate('fhcg', period)
        fhkt = simulation.calculate('fhkt', period)
        fhku = simulation.calculate('fhku', period)
        fhlh = simulation.calculate('fhlh', period)
        fhli = simulation.calculate('fhli', period)
    ...
    def formula_2016_01_01(self, simulation, period):
        ...
        fhce = simulation.calculate('fhce', period)
        fhcg = simulation.calculate('fhcg', period)
        fhci = simulation.calculate('fhci', period)
        fhcj = simulation.calculate('fhcj', period)
        fhck = simulation.calculate('fhck', period)
        fhcm = simulation.calculate('fhcm', period)
        fhcn = simulation.calculate('fhcn', period)
        fhco = simulation.calculate('fhco', period)
        fhcp = simulation.calculate('fhcp', period)
        fhcr = simulation.calculate('fhcr', period)
        fhcs = simulation.calculate('fhcs', period)
        fhct = simulation.calculate('fhct', period)
        fhcu = simulation.calculate('fhcu', period)
        fhcw = simulation.calculate('fhcw', period)
        fhnu = simulation.calculate('fhnu', period)
        fhnv = simulation.calculate('fhnv', period)
        fhnw = simulation.calculate('fhnw', period)
        fhny = simulation.calculate('fhny', period)
...
class duflot(Variable):
    ...
    def formula_2014_01_01(self, simulation, period):
        ...
        f7gh = simulation.calculate('f7gh', period)
        f7gi = simulation.calculate('f7gi', period)
        f7qa = simulation.calculate('f7qa', period)
        f7qb = simulation.calculate('f7qb', period)
        f7qc = simulation.calculate('f7qc', period)
        f7qd = simulation.calculate('f7qd', period)
        P = simulation.parameters_at(period.start).impot_revenu.reductions_impots.duflot
...
class rpinel(Variable):
    ...
    def formula_2015_01_01(self, simulation, period):
        ...
        2015
        '''
        f7ai = simulation.calculate('f7ai', period)
        f7bi = simulation.calculate('f7bi', period)
        f7ci = simulation.calculate('f7ci', period)
        f7di = simulation.calculate('f7di', period)
        f7ek = simulation.calculate('f7ek', period)
        f7el = simulation.calculate('f7el', period)
        f7qa = simulation.calculate('f7qa', period)
        f7qb = simulation.calculate('f7qb', period)
    ...
    def formula_2016_01_01(self, simulation, period):
        ...
        f7ai = simulation.calculate('f7ai', period)
        f7bi = simulation.calculate('f7bi', period)
        f7bz = simulation.calculate('f7bz', period)
        f7ci = simulation.calculate('f7ci', period)
        f7cz = simulation.calculate('f7cz', period)
        f7di = simulation.calculate('f7di', period)
        f7dz = simulation.calculate('f7dz', period)
        f7ek = simulation.calculate('f7ek', period)
        f7el = simulation.calculate('f7el', period)
        f7ez = simulation.calculate('f7ez', period)
        f7qa = simulation.calculate('f7qa', period)
        f7qb = simulation.calculate('f7qb', period)
        f7qc = simulation.calculate('f7qc', period)
        f7qd = simulation.calculate('f7qd', period)
...
class scelli(Variable):
    ...
    def formula_2016_01_01(self, simulation, period):
        ...
        f7li = simulation.calculate('f7li', period)
        f7lj = simulation.calculate('f7lj', period)
        f7lk = simulation.calculate('f7lk', period)
        f7ll = simulation.calculate('f7ll', period)
        f7lm = simulation.calculate('f7lm', period)
        f7ln = simulation.calculate('f7ln', period)
        f7lo = simulation.calculate('f7lo', period)
        f7lp = simulation.calculate('f7lp', period)
        f7ls = simulation.calculate('f7ls', period)
        f7lt = simulation.calculate('f7lt', period)
        f7lx = simulation.calculate('f7lx', period)
        f7lz = simulation.calculate('f7lz', period)
        ...
        f7yr = simulation.calculate('f7yr', period)
        f7ys = simulation.calculate('f7ys', period)
        f7yt = simulation.calculate('f7yt', period)
        f7yu = simulation.calculate('f7yu', period)
        f7yv = simulation.calculate('f7yv', period)
        f7yw = simulation.calculate('f7yw', period)
        f7yx = simulation.calculate('f7yx', period)
        f7yy = simulation.calculate('f7yy', period)
        f7yz = simulation.calculate('f7yz', period)
